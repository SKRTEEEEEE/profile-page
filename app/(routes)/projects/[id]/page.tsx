import AutoplaySliderTest from "@/components/main/autoplay-slider-test";
...
export default function ProjectsDynPage({ params }: { params: { id: number } }) {
  ...
  return (
    <main className="min-h-dvh w-dvw flex flex-col items-start justify-center">
      ...
      <h1 className="px-4 xl:pl-24 text-3xl">{project.title}</h1>
      <p className="px-4 xl:pl-24 ">{project.description}</p>

      <section className="mx-4 xl:ml-24 flex gap-4 rounded-2xl bg-secondary-800 xl:my-2 px-2">
      ...
      <article className="">
        ...
        <section className="flex w-dvw flex-col align-center">
            ...
            <h3 className="absolute ml-4 xl:ml-12 text-xl text-primary-400 font-bold">Frontend</h3>
            <AutoplaySliderTest data={project.technologies.frontend}/>
        </section>
        <section>
