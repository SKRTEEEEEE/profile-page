      ...
      <ul className="w-11/12 xl:w-9/12 flex flex-col gap-4">
        {currentProjects.map((data: Project) => (
          <li className="flex justify-between" key={data.id}>
            ...
            <div className='flex'>
              {data?.urlDemo && <Link className='mr-4 px-2 py-1 border-2 border-primary-ceo-200 rounded-md bg-secondary-ceo-300/50 hover:bg-secondary-ceo-600 hover:border-primary-ceo-400/80 sm:inline hidden' href={data.urlDemo} target='_blank'>{t("ceo.proyectos.main.ul.buttons.url_demo.0")} <span className="hidden xl:inline">{t("ceo.proyectos.main.ul.buttons.url_demo.1")}</span>🧑‍💻</Link>}
              {data?.urlGithub && <Link className='mr-4 px-2 py-1 border-2 border-primary-ceo-200 rounded-md bg-secondary-ceo-300/50 hover:bg-secondary-ceo-600 hover:border-primary-ceo-400/80 sm:inline hidden' href={data.urlGithub} target='_blank'><span className="hidden xl:inline">{t("ceo.proyectos.main.ul.buttons.url_github.0")} </span>{t("ceo.proyectos.main.ul.buttons.url_github.1")}📄</Link>}
              <LinkLocale className='px-2 py-1 border-2 border-primary-ceo-200 rounded-md bg-secondary-ceo-400/30 hover:bg-secondary-ceo-600 hover:border-primary-ceo-400/80' href={{ pathname: `/ceo/proyectos/[id]`, params: { id: data.id } }}>{t("ceo.proyectos.main.ul.buttons.url_project.0")} <span className="hidden lg:inline mr-4">{t("ceo.proyectos.main.ul.buttons.url_project.1")}</span>➡️</LinkLocale></div>
          </li>
        ))}
      </ul>
