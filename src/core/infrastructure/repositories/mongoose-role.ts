...
class MongooseRoleRepository extends MongooseRepository<RoleBase, Role, RoleDocument> implements RoleRepository {
    constructor() {
      super(new RoleModel); // Pasa el modelo como argumento
    }
        async findDelete(filter?: FilterQuery<any> | null | undefined, options?: QueryOptions<any> | null | undefined): Query<any, any, {}, any, "findOneAndDelete", {}> {
        await this.connect()
        return await RoleModel.findOneAndDelete(filter, options)
