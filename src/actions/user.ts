...
import {
  ...
  giveRoleUCMongoose,
  resendVerificationEmailCMongoose,
  updateUserFormCMongoose,
} from "@/core/presentation/controllers/user";
...
//   await updateUserFormCMongoose({payload,user: {
//     id,
...
export async function updateUserSolicitud({
  id,
  solicitud,
}: {
  id: string;
  solicitud: RoleType.ADMIN | null;
}) {
  // return await mongooseUpdateUserByIdUC(id, {solicitud})
  if (!solicitud)
    throw new InputParseError(
      updateUserSolicitud,
      "Solicitud no puede ser nulo"
    );
...
export async function deleteUser(
  ...
  if (!res.success)
    throw new DatabaseActionError("apiDeleteUserUC", deleteUser, {
      optionalMessage: res.message,
    });
...
export async function giveRole(
  ...
  if (!res.success)
    throw new DatabaseActionError("apiGiveRoleToUserUC", giveRole, {
      optionalMessage: res.message,
    });
